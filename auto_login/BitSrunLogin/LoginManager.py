...
class LoginManager:
	...
	def _resolve_ip_from_login_page(self):
		# self.ip = "10.1.113.96"
		self.ip = re.search(r'ip\s*:\s*"(\d{1,3}(?:\.\d{1,3}){3})"', self._page_response.text).group(1)
	...
	def _get_challenge(self):
		...
		params_get_challenge = {
			"callback": "jQuery112403484635456370784_1744788281096", # This value can be any string, but cannot be absent
			"username": self.username,
			"ip": self.ip,
			"_": tt
		}

		self._challenge_response = requests.get(self.url_get_challenge_api, params=params_get_challenge)
